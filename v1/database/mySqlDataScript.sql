...
FROM classifiers as cl
        CROSS JOIN (
    SELECT 'TRAVEL_MEDICAL'as ic, 'Travel policy medical risk type' as description union all
    SELECT 'TRAVEL_CANCELLATION', 'Travel policy trip cancellation risk type'union all
    SELECT 'TRAVEL_LOSS_BAGGAGE', 'Travel policy baggage lose risk type'union all
    SELECT 'TRAVEL_THIRD_PARTY_LIABILITY', 'Travel policy third party liability risk type' union all
    SELECT 'TRAVEL_EVACUATION', 'Travel policy evacuation risk type' union all
